...
mkdir "${path}"/customizo/resources/source-files
7z -y x ${iso} -o"${path}"/customizo/resources/source-files
mkdir "${path}"/customizo/resources/source-files/custom
touch user-data
touch "${path}"/customizo/resources/source-files/custom/meta-data
...
${tab}${tab}- cp /cdrom/custom/docker_setup.sh /target/root
${tab}${tab}- curtin in-target --target=/target -- chmod +x /root/docker_setup.sh
...
mv user-data "${path}"/customizo/resources/source-files/custom/
...
for ((i=0; i<$length; i++)) do
	sed -i "/packages/a \ \ \ \ - ${packages[$i]}" "${path}"/customizo/resources/source-files/custom/user-data
done

# Add packages required for Docker (ca-certificate, curl, gnupg, lsb-release)
sed -i "/packages/a \ \ \ \ - lsb-release" "${path}"/customizo/resources/source-files/custom/user-data
sed -i "/packages/a \ \ \ \ - gnupg" "${path}"/customizo/resources/source-files/custom/user-data
sed -i "/packages/a \ \ \ \ - curl" "${path}"/customizo/resources/source-files/custom/user-data
sed -i "/packages/a \ \ \ \ - ca-certificates" "${path}"/customizo/resources/source-files/custom/user-data
